...
args = getResolvedOptions(sys.argv, [
    ...
    'silver_bucket',      # Silver S3 bucket for direct read
...
def read_silver_statistics():
    """
    Read clean statistics data directly from S3
    Using spark.read instead of Glue catalog
    Avoids Glue auto-adding partition columns as duplicates
    which was causing duplicate region column error
    """
    silver_path = f"s3://{args['silver_bucket']}/youtube/clean_statistics/"
    logger.info(f"Reading silver statistics from: {silver_path}")

    try:
        # ---- Read directly from S3 not Glue catalog ----
        # Avoids partition column auto-addition issue
        df = spark.read.parquet(silver_path)

        # ---- Drop region if exists ----
        # region is a partition folder not needed for analytics
...
def read_silver_reference():
    """
    Read clean reference data directly from S3
    Using spark.read instead of Glue catalog
    Avoids Glue auto-adding partition columns as duplicates
    which was causing duplicate region column error
    """
    silver_path = f"s3://{args['silver_bucket']}/youtube/raw_statistics_reference_data/"
    logger.info(f"Reading silver reference data from: {silver_path}")
    ...
    try:
        ...
        # ---- Drop region if exists ----
        # region is a partition folder not needed for category lookup
        if 'region' in df.columns:
            df = df.drop('region')
...
#     """
#     Remove duplicate columns that Glue adds automatically
#     from partition keys when reading from catalog
#     Glue auto-adds partition columns which can create duplicates
...
#     Deduplicates columns and drops region to prevent join conflict
#     """
...
#         # ---- Remove duplicate columns from Glue partition auto-add ----
#         df = deduplicate_columns(df)

#         # ---- Drop region not needed for analytics ----
#         if 'region' in df.columns:
#             df = df.drop('region')
#             logger.info("Dropped region column from statistics")
...
#         # ---- Drop region not needed for category lookup ----
#         if 'region' in df.columns:
#             df = df.drop('region')
#             logger.info("Dropped region column from reference data")
